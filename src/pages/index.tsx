...
const IndexPage: React.FC = () => {
  ...
  const handleScan = async (result: any) => {
    if (result && !scanningRef.current) {
      ...
      try {
        const response = await axios.post('/api/check-in', { id: result?.text });
  ...
  return (
    ...
    >
      ...
      {nama && kelas && (
        <Box mt={2}>
          <Typography variant="body1"><strong>Nama:</strong> {nama}</Typography>
          <Typography variant="body1"><strong>Kelas:</strong> {kelas}</Typography>
        </Box>
      )}
  ...
  <DialogTitle style={{ backgroundColor: loading ? '#2196F3' : dialogType === 'success' ? '#4CAF50' : '#f44336', color: '#fff' }}>
  ...
  color: '#fff' 
    ...
    {loading ? (
      ...
      <>
        ...
        {dialogType === 'success' && (
          <Box mt={2}>
            <Typography><strong>Nama:</strong> {nama}</Typography>
            <Typography><strong>Kelas:</strong> {kelas}</Typography>
          </Box>
        )}
      </>
    )}
